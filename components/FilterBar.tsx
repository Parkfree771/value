...
const FilterBar = memo(function FilterBar({ onFilterChange }: FilterBarProps) {
  ...
  return (
    <div className="bg-[var(--theme-bg-card)] border-2 border-[var(--theme-border-muted)] rounded-xl shadow-sm mb-6 transition-colors">
      {/* 모바일: 한 줄 축약 버전 */}
      <div className="block md:hidden">
        <div className="p-3 flex items-center gap-2">
          ...
          <button
            ...
            className="btn-primary relative px-4 py-2 text-xs sm:text-sm flex items-center gap-1"
          >
            ...
            필터
            {activeFilterCount > 0 && (
              <span className="absolute -top-1 -right-1 w-5 h-5 bg-[var(--theme-accent)] text-white text-xs font-sans border-2 border-[var(--theme-accent-dark)] rounded-full flex items-center justify-center">
                {activeFilterCount}
              </span>
            )}
          </button>
        ...
        {/* 확장된 필터 옵션 (모바일) */}
        {showAllFilters && (
          <div className="px-3 pb-3 space-y-2 border-t-2 border-[var(--theme-border-muted)] pt-3">
            <select
